...
namespace DAL
{
    public class ChiTietHoaDonDAL:DangNhapDAL
    {
        ...
        {
            var cmd = new SqlCommand("SELECT HD.maHD, CTHD.maSP, SP.tenSP, CTHD.soLuong,CTHD.giaBan,CTHD.maKM, CTHD.tongTien FROM hoaDon HD JOIN CT_HoaDon CTHD ON HD.maHD = CTHD.maHD JOIN sanPham SP ON CTHD.maSP = SP.maSP WHERE HD.maHD = @maHD", _conn);
